...
class Parsexinshipu:
	...
	def getDataById(self,id):
		conn = sqlite3.connect("cook.db")
		sql = "SELECT * FROM xinshipu where id = '%d'" %(id)
		cur = conn.cursor()
		cur.execute(sql)
		data =  cur.fetchone()
		cur.close()
	...
	def parseHtml(self,id):
		...
		# exit(1)
		data =[_title,_img,_summary,_cailiao,_content,_title1,_url,_tag,_cateid,_created]
	...
	def insertMongo(self,data):
		...
		cooks = db.cooks
		cook = cooks.find_one({"url":data[6],"cateid":data[8]})	#判断之前的数据存在没有
		if cook == None:
			cook = {"title":data[0],"img":data[1],"summary":data[2],"cailiao":data[3],"content":data[4],"title1":data[5],"url":data[6],"tag":data[7],"cateid":data[8],"created":data[9]}
